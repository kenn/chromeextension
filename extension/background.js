        ...
        function handleDOMOperation(operation) {
          switch (operation.action) {
            ...
            case 'removeAttribute':
              const targetForRemoveAttr = document.querySelector(
                operation.selector
              )
            ...
            case 'addClass':
              const targetForAddClass = document.querySelector(
                operation.selector
              )
            ...
            case 'removeClass':
              const targetForRemoveClass = document.querySelector(
                operation.selector
              )
            ...
            case 'toggleClass':
              const targetForToggleClass = document.querySelector(
                operation.selector
              )
            ...
            case 'appendChild':
              ...
              const child = document.querySelector(
                `[data-mcp-id="${operation.elementId}"]`
              )
              if (!parent || !child)
...
async function handleCaptureScreenshot({
  ...
  return await chrome.tabs.captureVisibleTab(tab_id ? tab_id : null, options)
}
...
setInterval(() => {
  if (lastMessage && Date.now() - lastMessage.timestamp > 3600000) {
    // 1時間以上経過したメッセージをクリア
    lastMessage = null
  }
}, 3600000) // 1時間ごとにチェック
